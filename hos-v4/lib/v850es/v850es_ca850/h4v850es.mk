...
#		ライブラリメイクファイル V850ES互換コア用
#
#		コンパイルには NEC Electronics 提供の CA850 コンパイラを利用します。
#		CA850環境のあるディレクトリを TOOLDIR にて定義してください。
#
...
SRCDIR     = $(HOSROOT)/src
PACDIR     = $(SRCDIR)/v850es
PACASMDIR  = $(PACDIR)/v850es_ca850
MKNLDIR    = $(SRCDIR)/mknl
...
# ツール
CC     = $(TOOLDIR)/bin/ca850
ASM    = $(TOOLDIR)/bin/as850
LIBR   = $(TOOLDIR)/bin/ar850
RANLIB = $(TOOLDIR)/bin/ld850

# オプション
CFLAGS = -cpu F3716 -I$(INCDIR) -D__V850ES__ -c -Xcxxcom -Xpro_epi_runtime=off -Xr -O
AFLAGS = -cpu F3716 -I $(INCDIR)

# ターゲット
TARGET  = libh4v850es.a
...
pacctx.o: $(PACASMDIR)/pacctx.S
	$(ASM) $(AFLAGS) -o pacctx.o $(PACASMDIR)/pacctx.S

pacint.o: $(PACASMDIR)/pacint.S
	$(ASM) $(AFLAGS) -o pacint.o $(PACASMDIR)/pacint.S

pacimsk.o: $(PACDIR)/pacimsk.c
	$(CC) $(CFLAGS) $(PACDIR)/pacimsk.c

hospac.o: $(PACDIR)/hospac.c
	$(CC) $(CFLAGS) $(PACDIR)/hospac.c
...
install:
#	cp $(TARGET) $(TOOLDIR)/lib850/r22
#	cp $(TARGET) $(TOOLDIR)/lib850/r26
	cp $(TARGET) $(TOOLDIR)/lib850/r32
