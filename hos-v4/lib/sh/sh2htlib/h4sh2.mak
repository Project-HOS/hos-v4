# -----------------------------------------------------------------------------
#  HOS-V4                                                                      
#   ライブラリメイクファイル SH2 日立評価版 Ver.5 Cコンパイラ用                
#                                                                              
...
# にて行っております。
# 秋月 Cコンパイラのインストール環境に合わせて、SHC_INC、SHC_LIBなどの
# 環境変数を正しく設定してからご利用ください。


# パス定義
HOSROOT    = ..\..\..			#ソースコードを CVSでディレクトリごと取得した場合はこのままで構いません
INCDIR     = $(HOSROOT)\include
SRCDIR     = $(HOSROOT)\src
PACDIR     = $(SRCDIR)\sh
PACASMDIR  = $(SRCDIR)\sh\sh2ht
MKNLDIR    = $(SRCDIR)\mknl
