...
static UH Tsk1Stack[(256 + 1) / 2];
static UH Tsk2Stack[(256 + 1) / 2];
static UH Tsk3Stack[(256 + 1) / 2];
...
UH kernel_int_stack[(128 + 1) / 2];				/* 割り込み用スタック */
const UH *kernel_int_sp = &kernel_int_stack[(128 + 1) / 2];

const VP   mknl_int_stk   = kernel_int_stack;	/* 割り込みスタックアドレス */
const SIZE mknl_int_stksz = 256;				/* 割り込みスタックサイズ */

T_KERNEL_INTCB  kernel_intcb_tbl[32];			/* 割り込み管理コントロールブロックテーブル */
const INT       kernel_intcb_cnt = 32;			/* 最大割り込み番号数 */

const UH *kernel_sys_sp = (UH *)0x240000;		/* スタックポインタの初期値 */
